				...
				<el-tabs type="border-card">
					<el-tab-pane label="基础设置">
						...
						<el-form-item label="别名">
							<el-input size="small" v-model="form.path" placeholder="别名用于url美化"></el-input>
						</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getArticleInfo() {
				...
				ArticleInfo(id, {}).then(res => {
					...
					this.form = {
						...
						is_top: ret.is_top,
						is_nice: ret.is_nice,
						is_index: ret.is_index,
						sort: ret.sort,
						state: ret.state,
						thumb: ret.thumb,
						thumb_ext: ret.thumb_ext,
						seo_title: ret.seo_title,
						seo_desc: ret.seo_desc,
						seo_key: ret.seo_key,
						path: ret.path,
						token: ret.token
					}
					...
					if (ret.Tags.length > 0) {
						var tgs = []
						ret.Tags.forEach(function(item, index) {
							tgs.push(item.tag_name)
						})
						this.form.tags = tgs.join(",")
					}

					//设置缩略图
					this.imageUrl = ret.thumb
					//设置图片列表
					if (ret.thumb_ext != "") {
						var that = this
						var tlist = ret.thumb_ext.split(",")
						tlist.forEach(function(item, index) {
							var lst = {
								name: "",
								url: item
							}
							that.uploadList.push(lst)
						})
					}

					//获取上级分类
					if (ret.category) {
						GetAllCategoryId(ret.category.id, {}).then(res => {
			...
			handleRemove(file, fileList) {
				...
				// console.log("文件", file, "文件列表", fileList);
			},
